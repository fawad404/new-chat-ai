      ...
      {columns.map(({ title, items, callToAction }, index) => (
        <div
          ...
          className={`relative mx-auto w-full min-w-fit max-w-3xl select-none border-r border-solid border-gray-300 px-4 py-4 first-of-type:sticky first-of-type:left-0 first-of-type:z-10 first-of-type:w-auto ${
            hasBackground ? 'first-of-type:bg-primary-50' : 'first-of-type:bg-white'
          } first-of-type:pl-6 last-of-type:border-none md:px-5 md:first-of-type:w-full md:first-of-type:pl-5`}
        >
          <h3
            className={`mb-4 border-b border-solid border-gray-300 pb-4 text-lg font-medium uppercase leading-6 text-gray-900 ${
          ...
          {items &&
            items.map(({ title: title2 }, index2) => (
              <div
                ...
                className={`leading-7 text-gray-600 ${index === 0 ? 'text-left' : 'text-center'}`}
              >
